...
pis_compositorErrors pis_addNewSlide(pis_slides_s **ret, uint32_t dissolveTime, uint32_t duration, const char *name)
{
		...
		switch(current->mediaElement.mediaType){
			case pis_MEDIA_IMAGE:
				...
				if(dataImg->res == 0)
					...
					sImage *img = loadImage(dataImg->filename,
							dataImg->maxWidth * pis_compositor.screenWidth,
							dataImg->maxHeight * pis_compositor.screenHeight);
					...
					if(ret != 0)
						printf("Error writing image data to dispmanx resource %d\n", ret);
					}else{
			...
			case pis_MEDIA_TEXT:
				...
				if(dataTxt->res == 0)
				{
					...
					uint32_t *temp= new uint32_t[pis_compositor.screenWidth*pis_compositor.screenHeight];
					...
					VC_RECT_T rectTxt = {0,0,(int32_t)pis_compositor.screenWidth,(int32_t)pis_compositor.screenHeight};
...
void pis_initDispmanx()
{
    ...
    alphaDissolve.flags = (DISPMANX_FLAGS_ALPHA_T)(DISPMANX_FLAGS_ALPHA_FROM_SOURCE | DISPMANX_FLAGS_ALPHA_MIX);
...
//	printFontList();
	...
	uint32_t *temp = new uint32_t[pis_compositor.screenHeight*pis_compositor.screenWidth];
	uint32_t x,y;
	...
	dispElement2 = vc_dispmanx_element_add(update,display,2001,&screenRect,dissolveDispRes,&srcRect,
				DISPMANX_PROTECTION_NONE,&alphaDissolve,NULL,DISPMANX_NO_ROTATE);
